...
def integrand_f_numerator(Ee,phase,experiment,E_i,c_i,T):
    """
    Integrand function in the definition of the correction function f
    ...
    return eta*res_kernel*g*xsec*j

def integrate_f_numerator(phase,experiment,E_i,c_i,T):
    """
    Evaluates the integral of the integrand function at the numerator of the f_function definitions.
    The integrand function is defined in integrand_f_numerator
    """
    Ee_min = 2.5    # Mev
    Ee_max = 70    # Mev
    integral_value,error_value = spi.quad(integrand_f_numerator, Ee_min, Ee_max, args=(phase, experiment, E_i, c_i, T), epsabs=1e-100,epsrel=1e-10) # Rimosso limit = 100. Perché c'era?
    
    return integral_value,error_value

def calculate_f_denominator(phase,experiment,E_i,c_i,T):
    """
    Evaluates the denominator of the f_function.
    """
...
def run(folder_path = "./tables/f_functions/",experiment_list=("k2","im","bk"),test_mode=False):
    ...
    temperatures = {
                ...
                "c" :   np.arange(  1.0,  10.0+function_temperature_step,    function_temperature_step)      # Cooling
                }


    # f is evaluated for each experiment, each phase (accretion and cooling) and each event
    for experiment in experiment_list:
        for n_event,time,E_i,theta_i,bkg in data[experiment]:
            for phase in phases:
                
                # Open a (new) file associated to event
                filename = f"f_func_{experiment}_{phase}_ev{n_event:02d}.tsv"
                f_function_file = open(folder_path + filename, "w")
                f_function_file.write("T[MeV]\tf_value\tf_error\n")
                
                for T in temperatures[phase]:

                    # evaluate f function
                    cos                     = lambda theta : np.cos(theta*np.pi/180)
                    f_numerator,integ_error = integrate_f_numerator(phase,experiment,E_i,cos(theta_i),T)
                    f_denominator           = calculate_f_denominator(phase,experiment,E_i,cos(theta_i),T)
                    f_value_at_T            = f_numerator/f_denominator
                    f_integ_error           = integ_error/f_denominator

                    # Warning if relative error is more than 1 over 1 milion
                    if f_integ_error/f_value_at_T > 1e-6:
                        print(f"[Warning] In file {filename} found error of {f_integ_error:.1e} for T = {T:.3f}")

                    # write into file
                    string_to_file =  f"{T:.3f}\t{f_value_at_T:.10f}\t{f_integ_error:.1e}\n"
