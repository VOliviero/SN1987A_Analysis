...
class Signal_reader:
    ...
    def __init__(self,experiment,phase,event,folder_path=None):
        ...
        if folder_path == None:
            folder_path = dirname(dirname(abspath(__file__)))+"/tables/Signal_tables/"

        self.folder_path = folder_path + "/"
        self.files = [f for f in listdir(self.folder_path) if isfile(join(self.folder_path, f))]

        filename = lambda experiment,phase,event : f"signal_func_{experiment}_{phase}_ev{event:02d}.tsv"
        ...
        self.signal_value= self.data["signal_value"].to_numpy()
        

    '''def eval(self,T):
        return np.interp(T,self.T,self.signal_value,left=np.nan,right=np.nan)'''
    
    def eval(self, T):
    # Creiamo una funzione di interpolazione utilizzando interp1d
        interp_func = scipy.interpolate.interp1d(self.T, self.signal_value, kind='cubic',bounds_error=False, fill_value=0)
...
class manager():
    ...
    def eval(self,T,experiment,phase,event):
        ...
        if key in self.readers.keys():
            ...
            self.readers[key] = Signal_reader(experiment,phase,event,self.folder_path)
            return self.readers[key].eval(T)
    ...
    for i in range(2):
        for j in range(3):
            ...
            for n,_,_,_,_ in expdata.data[experiment]:
                f_func = Signal_reader(experiment,phase,n)
